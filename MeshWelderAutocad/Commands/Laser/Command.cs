...
namespace MeshWelderAutocad.Commands.Laser
{
    internal class Command
    {
                    ...
                    using (DocumentLock docLock = newDoc.LockDocument())
                    {
                        using (Transaction tr = _db.TransactionManager.StartTransaction())
                        {
                            ...
                            if (_panel.EmbeddedParts6.Count != 0)
                                CreateEmbeddedDetail6Or9($"ЗД 1.6", _panel.EmbeddedParts6, _widthDetail6);
                            ...
                            if (_panel.EmbeddedParts9.Count != 0)
                                CreateEmbeddedDetail6Or9($"ЗД 1.9", _panel.EmbeddedParts9, _widthDetail9);
        ...
        private static void CreateEmbeddedDetail6Or9(string layerName, List<EmbeddedPart> parts, double widthDetaill)
        {
            CreateLayer(_db, layerName);
            ObjectId layerId = _layerTable[layerName];
